...
global.SvgLayer = require('./svglayer');
...
// var w = 4000 * 2,
//     h = 2500 * 2,
//     url = 'https://gis.eptw.nl/data/pnid.svg';
...
var svg = global.s = new SvgOverlay(document.querySelector('.container svg')).addTo(map);
...
},{"./svglayer":3,"./svgoverlay":4,"leaflet":1}],3:[function(require,module,exports){
'use strict';
...
function getBBox(svg) {
  var bbox = svg.getAttribute('viewBox').split(' ').map(parseFloat);
...
module.exports = SvgLayer.extend({
  ...
  load: function load() {
    xhr.get(this._url, function (svg) {}, this);
  },
  ...
  _endPathZoom: function _endPathZoom() {
    // var scale = this._map.getZoom();
    // var offset = this.getOriginalSize()
    //   ._add(this._getViewport().min.multiplyBy(scale));
    ...
    // //   L.DomUtil.getTranslateString(offset) + ' scale(' + scale + ') ';
    // // var offset = this._map.project(this._bounds.getSouthWest());
  ...
  _reset: function _reset() {
    ...
    var scale = Math.pow(2, this._map.getZoom() - 1);
    var topLeft = this._map.latLngToLayerPoint(this._bounds.getNorthWest());
    var size = this.getOriginalSize().multiplyBy(scale);

    console.log(topLeft, size, scale);

    // if (scale > 1) {
    //   var origin = topLeft.add(size.divideBy(2));
    //   //var origin = size.divideBy(2);
    //   console.log(origin, scale)

    //   image.style.transform = L.DomUtil.getScaleString(scale, origin);
    // } else {
